    ...
    if Value then
        task.spawn(function()
            local function GetFrozenDimension()
                return game:GetService("Workspace").Map:FindFirstChild("FrozenDimension") or 
                       game:GetService("Workspace"):FindFirstChild("FrozenDimension") or
                       workspace._WorldOrigin.Locations:FindFirstChild('Frozen Dimension')
            end
            ...
            while _G.Auto and math.abs(LP.Character.HumanoidRootPart.Position.Y - 1000) > 5 do
                if GetFrozenDimension() then _G.Auto = false break end
                task.wait(0.1)
            end

            if _G.Auto and boat.Position.Z < 14238 then
                activeTween = TS:Create(boat, TweenInfo.new((Vector3.new(-13608, 1000, 14238) - boat.Position).Magnitude/250, Enum.EasingStyle.Linear), {CFrame = CFrame.new(-13608, 1000, 14238)})
                activeTween:Play()
