...
local Energy = char:WaitForChild("Energy").Value
...
_G.AutoRejoin = true
_G.autojump5 = true
_G.ChestCollected = _G.ChestCollected or 0
_G.CountedChests = _G.CountedChests or {}
  repeat
            wait()
            pcall(function()
                for i, v in getconnections(game:GetService("Players").LocalPlayer.PlayerGui["Main (minimal)"].ChooseTeam.Container[Config.TEAM].Frame.TextButton.Activated) do
                    v.Function()
                end
            end)
            task.wait(3)
...
BringEnemy = function()
	...
	_tp = function(target)
		...
		local tweenInfo = TweenInfo.new(distance / 370, Enum.EasingStyle.Linear)
			...
			pcall(function()  
				...
				for i = 1, #Chests do  
					...
					if Chest  
						...
						and (not SelectedIsland or Chest:IsDescendantOf(SelectedIsland)) then  
				...
				-- ✅ Còn rương → teleport + đếm (MỖI RƯƠNG 1 LẦN)
				if validChestCount > 0 and Nearest then  
					_tp(Nearest:GetPivot())  

					if not _G.CountedChests[Nearest] then
						_G.CountedChests[Nearest] = true
						_G.ChestCollected += 1
					end

					if SetStatus then  
						SetStatus(
							"AutoFarmChest: Nhặt rương (" ..
							_G.ChestCollected .. "/" ..
							tostring(getgenv().Config.chestBeforeHop or "∞") .. ")"
						)  
					end  
				...
				-- ❌ ĐỦ rương → hop (KHÔNG có FOD)
				if _G.ChestCollected >= (getgenv().Config.chestBeforeHop or math.huge)
					and not GetBP("Fist of Darkness") then  

					if SetStatus then  
						SetStatus("AutoFarmChest: Đủ rương → Đổi server")  
					end  
				...
				elseif GetBP("Fist of Darkness") then  
					if SetStatus then  
						SetStatus("AutoFarmChest: Có Fist of Darkness → Không đổi server")  
					end  
				end  
			end)  
		end  
...
spawn(function()
    while task.wait(0.2) do
        pcall(function()
            if _G.StopWhenfod then
                if GetBP("Fist of Darkness") then
                    _G.AutoFarmChest = false
                    _G.StopWhenfod = false -- stop hẳn
                end
            end
        end)
    end
...
task.spawn(function()
    while task.wait(0.2) do
        ...
        pcall(function()
            ...
            -- 🔹 STEP 2: ĐÁNH LAW LẤY CORE BRAIN
            if not GetBP("Core Brain") then
                _G.AutoLawKak = true
                return
            end
            ...
            -- 🔹 STEP 3: SUMMON RAID + BUY CYBORG
            local island = workspace:FindFirstChild("Map")
                and workspace.Map:FindFirstChild("CircleIsland")

            if not island then return end

            local raid = island:FindFirstChild("RaidSummon")
            local btn = raid and raid:FindFirstChild("Button")
                and raid.Button:FindFirstChild("Main")
			...
			if _G.DistroyHit then
				pcall(function()
					...
					for _, x in pairs(workspace["_WorldOrigin"]:GetChildren()) do
						if table.find(__Efeect, x.Name) then
							x:Destroy()
						end
					end
				end)
    ...
    while task.wait(0.2) do
        if _G.Stopcore then
            pcall(function()
                ...
                local hasFOD = GetBP("Fist of Darkness") ~= nil
                local hasCore = GetBP("Core Brain") ~= nil

                local btn = workspace.Map.CircleIsland
                    :WaitForChild("RaidSummon")
                    :WaitForChild("Button")
                    :WaitForChild("Main", 5)
                ...
                -- ✅ Fist of Darkness
                if hasFOD and not _G.FOD_Clicked then
                    if cd then
                        SetStatus("Auto Cyborg: FOD → Click Raid + Auto Law")
                        fireclickdetector(cd)
                        _G.FOD_Clicked = true
                        _G.AutoLawKak = true
                        return
                ...
                -- ✅ Core Brain
                if hasCore and not _G.Core_Clicked then
                    if cd then
                        SetStatus("Auto Cyborg: Core Brain → Buy Cyborg")
                        fireclickdetector(cd)
                        replicated.Remotes.CommF_:InvokeServer("CyborgTrainer", "Buy")
                        _G.Core_Clicked = true
                        return
                    end
                end

                -- ✅ dừng khi xong
                if not hasFOD and not hasCore then
                    SetStatus("Auto Cyborg: Done → Stop")
                    _G.Stopcore = false
                    _G.FOD_Clicked = false
                    _G.Core_Clicked = false
                end
            end)
        end
    end
	...
	while wait(Sec) do
		if _G.AutoLawKak then
			pcall(function()
				...
				--------------------------------------------------
				-- 1️⃣ Nếu chưa có Core Brain → chuẩn bị raid Law
				--------------------------------------------------
				if not GetBP("Core Brain") and not _G.LawRaidStarted then

					if SetStatus then
						SetStatus("Auto Law: Mua Microchip Law")
					end
					...
					if SetStatus then
						SetStatus("Auto Law: Start Law Raid")
					end

					-- click start raid
					local btn = workspace.Map.CircleIsland.RaidSummon.Button.Main
					local cd = btn and btn:FindFirstChildOfClass("ClickDetector")
					if cd then
						fireclickdetector(cd)
						_G.LawRaidStarted = true
					end
				...
				--------------------------------------------------
				-- 2️⃣ Tìm & đánh boss Order
				--------------------------------------------------
				if SetStatus then
					SetStatus("Auto Law: Tìm boss Order")
				end

				local v = GetConnectionEnemies("Order")
				if v then
					if SetStatus then
						SetStatus("Auto Law: Đánh Order")
					end
					...
					repeat
						...
						if v and v:FindFirstChild("Humanoid") and SetStatus then
							SetStatus(
								"Auto Law: Đang đánh Order | HP: " ..
								math.floor(v.Humanoid.Health)
							)
						end

						Attack.Kill(v, _G.AutoLawKak)

					until GetBP("Core Brain")
						or not v.Parent
						or v.Humanoid.Health <= 0
						or not _G.AutoLawKak
					...
					if GetBP("Core Brain") then
						if SetStatus then
							SetStatus("Auto Law: Đã có Core Brain")
						end
						_G.AutoLawKak = false
					end
				else
					if SetStatus then
						SetStatus("Auto Law: Chưa thấy Order – chờ spawn")
					end
				end
			end)
		end
...
spawn(function()
    ...
    while task.wait(0.5) do
        ...
        pcall(function()
            if game.PlaceId ~= World2 then
                teleported = true

                if SetStatus then
                    SetStatus("Traveling to Second Sea...")
                end
...
-- 🍎 Auto load 1 trái theo thứ tự (từ trên xuống dưới)
spawn(function()
    while task.wait(0.3) do
        if _G.Autofruit then
            ...
            for _, fruitName in ipairs(fruitOrder) do
                -- check trái có trong store không
                if GetIn(fruitName) then
                    replicated.Remotes.CommF_:InvokeServer("LoadFruit", fruitName)
                    task.wait(0.3)

                    _G.Autofruit = false -- ✅ lấy 1 trái xong thì tắt
                    break
                end
            end
...
spawn(function()
		while wait(Sec) do
			pcall(function()
				if _G.Auto_Cyborg then

					-- status: bắt đầu vòng Auto_Cyborg
					SetStatus("Auto_Cyborg: checking Alchemist/Wenlocktoad state")
					...
					if replicated.Remotes.CommF_:InvokeServer("Alchemist", "1") ~= 2 then
						...
						SetStatus("Auto_Cyborg: Alchemist state = " .. tostring(alcState))

						if alcState == 0 then
							SetStatus("Auto_Cyborg: sending Alchemist '2' to start quest")
							replicated.Remotes.CommF_:InvokeServer("Alchemist", "2")

						elseif alcState == 1 then
							SetStatus("Auto_Cyborg: collecting flowers (state 1)")

							if not plr.Backpack:FindFirstChild("Flower 1") and not plr.Character:FindFirstChild("Flower 1") then
								SetStatus("Auto_Cyborg: teleporting to Flower 1")
							...
							elseif not plr.Backpack:FindFirstChild("Flower 2") and not plr.Character:FindFirstChild("Flower 2") then
								SetStatus("Auto_Cyborg: teleporting to Flower 2")
							...
							elseif not plr.Backpack:FindFirstChild("Flower 3") and not plr.Character:FindFirstChild("Flower 3") then
								SetStatus("Auto_Cyborg: searching Swan Pirate for Flower 3")
								local v = GetConnectionEnemies("Swan Pirate")
								if v then
									repeat
										wait()
										if v and v:FindFirstChild("Humanoid") then
											SetStatus("Auto_Cyborg: fighting Swan Pirate | HP: " .. tostring(math.floor(v.Humanoid.Health)))
										end
										Attack.Kill(v, _G.Auto_Cyborg)
									until GetBP("Flower 3") or not v.Parent or v.Humanoid.Health <= 0 or _G.Auto_Cyborg == false

									SetStatus("Auto_Cyborg: finished Swan Pirate stage or stopped")
								else
									SetStatus("Auto_Cyborg: Swan Pirate not found → teleport fallback")
									_tp(CFrame.new(980.0985107421875, 121.331298828125, 1287.2093505859375))
								end
							end

						elseif alcState == 2 then
							SetStatus("Auto_Cyborg: finishing Alchemist quest (state 2 → 3)")
							replicated.Remotes.CommF_:InvokeServer("Alchemist", "3")
						end

					elseif replicated.Remotes.CommF_:InvokeServer("Wenlocktoad", "1") == 0 then
						SetStatus("Auto_Cyborg: Wenlocktoad state 0 → sending '2'")
						replicated.Remotes.CommF_:InvokeServer("Wenlocktoad", "2")

					elseif replicated.Remotes.CommF_:InvokeServer("Wenlocktoad", "1") == 1 then
						SetStatus("Auto_Cyborg: Wenlocktoad state 1 → load lowest fruit for raid")
						_G.Autofruit = true
						task.wait(0.5)
						...
						SetStatus("Auto_Cyborg: Wenlocktoad not active")
					end
				end
			end)
		end
...
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            local plr = game:GetService("Players").LocalPlayer
            if not plr or not plr:FindFirstChild("Data") then return end

            local race = plr.Data:FindFirstChild("Race")
            if race and race.Value == "Cyborg" then
                if not _G.Cymain then
                    _G.Cymain = true
                    _G.Auto_Cyborg = true
                    if SetStatus then
                        SetStatus("Cymain: Race Cyborg → Auto_Cyborg v3")
                    end
                end
            else
                if _G.Cymain then
                    _G.Cymain = false
                    _G.Auto_Cyborg = false
                    if SetStatus then
                        SetStatus("Cymain: Not Cyborg → Auto_Cyborg OFF")
                    end
                end
            end
        end)
...
-- số player tối đa cho phép
_G.LowerPlayer = _G.LowerPlayer or 6

local function GetPlayerCount(server)
	return server.playing or 0
end
	...
	task.spawn(function()
		...
		repeat
			...
			for _, server in pairs(Servers.data or {}) do
				local count = GetPlayerCount(server)

				-- ✅ SERVER ÍT NGƯỜI
				if count < _G.LowerPlayer then
					warn("[HopServer] → Server:", server.id, "| Players:", count)
...
-- TIME SAVE SYSTEM
----------------------------------------------------------
local folder = "BDUY_STATUS"
local file = folder.."/total_time.txt"
...
local function SaveTotalTime(t)
    writefile(file, tostring(t))
end

local startTick = tick()
local savedTime = LoadTotalTime()

----------------------------------------------------------
-- GUI ROOT
----------------------------------------------------------
local gui = Instance.new("ScreenGui")
gui.Name = "BDUY_STATUS_UI"
...
status.Position = UDim2.new(0.5, 0, 0.28, 0)
status.AnchorPoint = Vector2.new(0.5, 0.5)
status.BackgroundTransparency = 1
status.Text = "STATUS : IDLE"
status.Font = Enum.Font.Arcade -- Minecraft-style
status.TextSize = 32
...
local timeText = makeLine(0.55)
local totalText = makeLine(0.67)
local creditText = makeLine(0.79)

creditText.Text = "Fully Cyborg By Alya"
...
toggle.Image = "rbxassetid://129912951742770"
...
tStroke.Color = Color3.fromRGB(150,200,255)
tStroke.Thickness = 2
...
task.spawn(function()
    while task.wait(1) do
        local runTime = math.floor(tick() - startTick)
        local total = savedTime + runTime

        SaveTotalTime(total)

        timeText.Text = "Time : "..runTime.."s"
        totalText.Text = "Total Time : "..total.."s"
    end
end)

----------------------------------------------------------
-- STATUS API (DÙNG Ở SCRIPT KHÁC)
----------------------------------------------------------
